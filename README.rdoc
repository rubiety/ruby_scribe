...
Ruby Scribe attempts to intelligently format code (from an AST) much as a real developer would, through a series of configurable option.  Running this against the Rails codebase (as in, parsing the file and emitting it back out) shows very few differences.
